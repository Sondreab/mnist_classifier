...
Original file is located at
    https://colab.research.google.com/drive/1YRrWySjo5uLxhBbh-ZL6WX9ZleZeP5FO
"""
...
def check_gradient_ji(X, targets, w1, w2, epsilon, computed_gradient):
    """
    Computes the numerical approximation for the gradient of w1,
    ...
    for k in range(w1.shape[0]):
        for j in range(w1.shape[1]):
            new_weight1, new_weight2 = np.copy(w1), np.copy(w1)
    ...
    print("Max diff w1: {}".format(maximum_abosulte_difference))

def check_gradient_kj(X, targets, w1, w2, epsilon, computed_gradient):
    """
    Computes the numerical approximation for the gradient of w2,
    w.r.t. the input X and target vector targets.
    Asserts that the computed_gradient from backpropagation is 
    correct w.r.t. the numerical approximation.
    --
    ...
    for k in range(w2.shape[0]):
        for j in range(w2.shape[1]):
            ...
            new_weight1[k,j] += epsilon
            new_weight2[k,j] -= epsilon
            loss1 = cross_entropy_loss(X, targets, w1, new_weight1)
            loss2 = cross_entropy_loss(X, targets, w1, new_weight2)
            dw[k,j] = (loss1 - loss2) / (2*epsilon)
    maximum_abosulte_difference = abs(computed_gradient-dw).max()
    assert maximum_abosulte_difference <= epsilon**2, "Absolute error was: {}".format(maximum_abosulte_difference)
    print("Max diff w2: {}".format(maximum_abosulte_difference))
...
def softmax(a):
    ...
    """
    a_exp = np.exp(a)
    return a_exp / a_exp.sum(axis=1, keepdims=True) #axis = 1 has to be changed to axis = 0 for N=1
...
def forward(X, w1, w2):
    ...
    --
    X: shape: [batch_size, num_features(784+1)] numpy vector. Input batch of images
    w1: shape: [num_hidden_layer(J=64), num_features(I=784+1)] numpy vector. Weight from input->hidden layer
...
def gradient_descent(X, targets, w1, w2, v1, v2):
    """
    !!!! BEFORE MODIFICATION !!!!:
    ...
    outputs = forward(X, w1, w2) # shape = (num_samples, num_classes) 
    delta_k = - (targets - outputs) # (num_samples = N, num_classes = K)
    
    # Calculating new w_{kj} 
    aj = forward_ji(X, w1) # shape = (N, J) 
    
    
    dw2 = delta_k.T.dot(aj) # shape = (K, J)
    ...
    if should_check_gradient:
        check_gradient_kj(X, targets, w1, w2, 1e-2,  dw2)
        
    
    
    # Backprop w_{kj} to calculate new w_{ji}
    
    
    dfzj = hidden_activation_derivative(aj)
    
    delta_j = dfzj * delta_k.dot(w2) # (N, J) * (N, K) x (K , J) = (N, J) 
    dw1 = delta_j.T.dot(X) # (J, N) x (N, I)
    dw1 = dw1 / normalization_factor
    
   
    if should_check_gradient:
        check_gradient_ji(X, targets, w1, w2, 1e-2,  dw1)
...
def train_loop(w1, w2, v1, v2):
    training_it = 0
    for e in range(max_epochs): # Epochs
        #shuffle the training set after each epoch (3.a)
        if should_shuffle:
          X_epoch, Y_epoch = shufle_training_set(X_train,Y_train)
        else:
          X_epoch = X_train
          Y_epoch = Y_train
...
w_1, w_2 = train_loop(w_1, w_2, v_1, v_2)
